...
from keras.layers import Input, Dense, Flatten, Dropout, Conv2D, MaxPooling2D, BatchNormalization, Concatenate
...
if __name__ == "__main__":
    ...
    train_labels_oh_total = preprocess_labels(train_labels_total, smoothening_factor = 0.0)
    
    train_images_total = np.array(train_images_total, 'float32')
    #train_images_total = list(map(normalize_image, train_images_total))
    #split
    
    split_index = int(len(train_labels_total)*0.9)
...
#    train_images = apply_ZCA_matrix(train_images, zca)
    ...
    intermediate_model1 = ResNet50(input_tensor = input_layer, include_top = False, weights = 'imagenet', pooling = 'avg')
    intermediate_model2 = VGG16(input_tensor = input_layer, include_top = False, weights = 'imagenet', pooling = 'avg')
    
    features1 = intermediate_model1.output
    features2 = intermediate_model2.output
    
    
    #print(intermediate_model.summary())
    dp11 = Dropout(rate = 0.1)(features1)
    dp12= Dropout(rate = 0.1)(features2)
    dense11= Dense(256,activation = 'relu', name = 'dense11')(dp11)
    dense12 = Dense(64,activation = 'relu', name = 'dense12')(dp12)
    conc = Concatenate()([dense11, dense12])
    dense2 = Dense(64, activation = 'relu', name = 'dense2')(conc)
    dense3 = Dense(4, activation = 'softmax', name = 'dense3')(dense2)
    
    model = Model(inputs = input_layer, outputs = dense3)
    
    #model = Model(inputs = input_layer, outputs = dense4)
    
    for layer in intermediate_model1.layers:
        layer.trainable = False
    for layer in intermediate_model2.layers:
        layer.trainable = False
        
    
    
    early_stopping = EarlyStopping(monitor= 'loss', patience = 3, min_delta = 0.01)
    model_checkpoint = ModelCheckpoint('./models/ensemblevggres.h5', monitor = 'loss', save_best_only = True)
    ...
    #training
    hist1 = model.fit_generator(train_datagen.flow(np.array(train_images, 'float32'), train_labels_oh, batch_size = BATCH_SIZE), callbacks = callbacks,epochs = 20, steps_per_epoch = len(train_labels)//BATCH_SIZE)
...
#    pr_final_test_images  = list(map(normalize_image, pr_final_test_images))
    ...
    final_df.to_csv('./submissions/ensemblevggres.csv')
